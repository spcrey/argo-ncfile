...
class DrawContourImage(DrawFun):
    ...
    def __call__(self, fig, ax, frame, vmin, vmax):
        ...
        im = ax.contourf(frame, levels=self.level_num)
        ax.invert_yaxis()
        if self.colorbar_orientation:
            cbar = fig.colorbar(im, orientation=self.colorbar_orientation, aspect=self.colorbar_aspect)
...
def test():
    ...
    npdata_s = npdata_dict["s"][slice(None), slice(None), 0, slice(None)]
    npdata_b = npdata_dict["b"][slice(None), slice(None), 0, slice(None)]
